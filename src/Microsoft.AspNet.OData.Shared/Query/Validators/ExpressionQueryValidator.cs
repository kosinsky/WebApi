...
namespace Microsoft.AspNet.OData.Query.Validators
{
    ...
    public abstract class ExpressionQueryValidator
    {
        ...
        public virtual void ValidateNavigationPropertyNode(QueryNode sourceNode, IEdmNavigationProperty navigationProperty, ODataValidationSettings settings)
        {
            ...
            if (EdmLibHelpers.IsNotFilterable(navigationProperty, Property, StructuredType, Model,
        ...
        public virtual void ValidateSingleValuePropertyAccessNode(SingleValuePropertyAccessNode propertyAccessNode, ODataValidationSettings settings)
        {
            ...
            IEdmProperty currentProperty = propertyAccessNode.Property;
            ...
            if (propertyAccessNode.Source != null)
            {
                if (propertyAccessNode.Source.Kind == QueryNodeKind.SingleNavigationNode)
                {
                    ...
                    notFilterable = EdmLibHelpers.IsNotFilterable(currentProperty, singleNavigationNode.NavigationProperty,
                        singleNavigationNode.NavigationProperty.ToEntityType(), Model,
                        _defaultQuerySettings.EnableFilter);
                }
                else if (propertyAccessNode.Source.Kind == QueryNodeKind.SingleComplexNode)
                {
                    ...
                    notFilterable = EdmLibHelpers.IsNotFilterable(currentProperty, singleComplexNode.Property,
                        currentProperty.DeclaringType, Model, _defaultQuerySettings.EnableFilter);
                }
                else
                {
                    notFilterable = EdmLibHelpers.IsNotFilterable(currentProperty, Property, StructuredType, Model,
                        _defaultQuerySettings.EnableFilter);
                }
            }
            ...
            if (notFilterable)
            {
                throw new ODataException(Error.Format(SRResources.NotFilterablePropertyUsedInFilter, currentProperty.Name));
            }
        ...
        public virtual void ValidateSingleComplexNode(SingleComplexNode singleComplexNode, ODataValidationSettings settings)
        {
            ...
            // Check whether the property is filterable.
            IEdmProperty currentProperty = singleComplexNode.Property;
            if (EdmLibHelpers.IsNotFilterable(currentProperty, Property, StructuredType, Model,
                _defaultQuerySettings.EnableFilter))
            {
                throw new ODataException(Error.Format(SRResources.NotFilterablePropertyUsedInFilter, currentProperty.Name));
            }
        ...
        public virtual void ValidateCollectionComplexNode(CollectionComplexNode collectionComplexNode, ODataValidationSettings settings)
        {
            ...
            // Check whether the property is filterable.
            IEdmProperty currentProperty = collectionComplexNode.Property;
            if (EdmLibHelpers.IsNotFilterable(currentProperty, Property, StructuredType, Model,
                _defaultQuerySettings.EnableFilter))
            {
                throw new ODataException(Error.Format(SRResources.NotFilterablePropertyUsedInFilter, currentProperty.Name));
        ...
        {
            if (CurrentNodeCount >= validationSettings.MaxNodeCount)
            {
        ...
        {
            if (CurrentAnyAllExpressionDepth >= validationSettings.MaxAnyAllExpressionDepth)
            {
            ...
            CurrentAnyAllExpressionDepth++;
        }

        private void ExitLambda()
        {
            Contract.Assert(CurrentAnyAllExpressionDepth > 0);
            CurrentAnyAllExpressionDepth--;
        }
